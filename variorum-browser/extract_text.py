#!/usr/bin/env python3
"""
Extract prologue and chapter 1 from all Subcutanean EPUB files.
"""
...
def extract_chapter_from_epub(epub_path, chapter_file):
    ...
    try:
        ...
        print(f"Error extracting from {epub_path}: {e}")
        return None


def extract_all_versions():
    """Extract prologue and chapter 1 from all 25 EPUB versions."""
    ...
    for folder in epub_folders:
        ...
        # Extract prologue (ch002.xhtml) and chapter 1 (ch003.xhtml)
        prologue = extract_chapter_from_epub(epub_path, 'ch002.xhtml')
        chapter1 = extract_chapter_from_epub(epub_path, 'ch003.xhtml')

        if prologue and chapter1:
