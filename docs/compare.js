...
let globalMacrosCache = null; // Cache for macros defined in globals.txt

// Source Code Mode state
...
function convertSourceContentToParagraphs(content, sourceFileName = null) {
    ...
    function isFormattingOnlyBlock(text) {
        ...
        // Match blocks that are only formatting codes like {chapter/15}, {section_break}, {pp}, {part/...}, {vspace/N}
        // These are not actual rendered paragraphs
        const formattingOnlyPattern = /^\{(chapter|section_break|part|vspace|pp|verse|verse_inline)(\/[^}]*)?\}$/;
        return formattingOnlyPattern.test(trimmed);
    }

    // Helper to check if a block is a continuation alternative (starts with |)
    // These are alternative branches in conditional blocks like [@var1>text|@var2>alt text]
    // When split by double-newlines, |@var2>... becomes a separate block but it's actually
    // an alternative that only appears for specific seeds, not a separate content paragraph
    function isContinuationAlternative(text) {
        const trimmed = text.trim();
    ...
    blocks.forEach(block => {
        ...
        if (hasMacro) {
            ...
            // Content paragraph (not a comment, macro definition, formatting-only code, or continuation alternative)
...
// Helper to check if a variable's patterns exist in the rendered text for a specific version
function checkPatternsInVersionText(varName, targetVersionId) {
    if (!variableInfo || !variableInfo[varName]) return false;

    const patterns = variableInfo[varName].patterns?.[currentChapter] || [];
    if (patterns.length === 0) return false;

    const container = document.getElementById('comparison-display');
    if (!container) return false;

    // Normalize quotes/apostrophes for comparison
    ...
    // Get paragraphs belonging to the target version
    const versionPanels = container.querySelectorAll('.version-panel');

    let paragraphsToSearch = [];

    if (versionPanels.length >= 2) {
        // Side-by-side view: find the correct panel
        let panelIndex = -1;
        if (targetVersionId === versionA) panelIndex = 0;
        else if (targetVersionId === versionB) panelIndex = 1;
        else if (targetVersionId === versionC && versionPanels.length >= 3) panelIndex = 2;

        if (panelIndex >= 0 && versionPanels[panelIndex]) {
            paragraphsToSearch = versionPanels[panelIndex].querySelectorAll('p');
        }
    } else {
        // Unified/diff view: search all paragraphs (less precise but still useful)
        paragraphsToSearch = container.querySelectorAll('p, .comparison-paragraph, .source-paragraph');
    }

    // Check if any pattern matches any paragraph
    for (const para of paragraphsToSearch) {
        const text = para.textContent || '';
        const cleanText = normalizeQuotes(text.replace(/\s+/g, ' '));

        for (const pattern of patterns) {
            const cleanPattern = normalizeQuotes(pattern.replace(/\s+/g, ' ').trim());
            if (cleanText.includes(cleanPattern.substring(0, 50))) {
                return true;
            }
        }
    }
...
// Cache for chapter-local variable inference
const chapterLocalVarCache = new Map();

function clearChapterLocalVarCache() {
    chapterLocalVarCache.clear();
}

/**
 * Infer which chapter-local variables are active for a given version and chapter.
 * Uses pattern matching similar to global variable inference.
 * @param {string} versionId - The version to check
 * @param {string} chapterId - The chapter to check
 * @returns {Object} - Map of variable_name -> true for active variables
 */
function inferChapterLocalVariables(versionId, chapterId) {
    const cacheKey = `${versionId}-${chapterId}`;
    if (chapterLocalVarCache.has(cacheKey)) {
        return chapterLocalVarCache.get(cacheKey);
    }

    const activeVars = {};

    // Get the variable definitions for this chapter
    const varDefs = chapterVariables[chapterId];
    if (!varDefs || varDefs.length === 0) {
        chapterLocalVarCache.set(cacheKey, activeVars);
        return activeVars;
    }

    // Get the text for this version and chapter
    let chapterText = '';
    const versionData = allVersions[versionId] || customVersions[versionId];
    if (versionData && versionData[chapterId]) {
        const paragraphs = versionData[chapterId];
        if (Array.isArray(paragraphs)) {
            chapterText = paragraphs.map(p =>
    ...
    // Normalize quotes for pattern matching
    const normalizeQuotes = (str) => str
        .replace(/[\u2018\u2019\u201B\u2032]/g, "'")
        .replace(/[\u201C\u201D\u201F\u2033]/g, '"')
        .replace(/[\u2014]/g, '--')
    ...
    const normalizedText = normalizeQuotes(chapterText);

    // Process each variable definition
    for (const varDef of varDefs) {
        if (!varDef.patterns || Object.keys(varDef.patterns).length === 0) continue;

        if (varDef.is_group && varDef.variables.length > 1) {
            // Mutually exclusive group - find best match
            let bestMatch = null;
            let bestScore = 0;
            ...
            for (const varName of varDef.variables) {
                ...
                for (const pattern of patterns) {
                    const normalizedPattern = normalizeQuotes(
                        pattern.replace(/<[^>]+>/g, '').toLowerCase().trim()
                    );

                    if (normalizedPattern.length >= 3 && normalizedText.includes(normalizedPattern)) {
            ...
            if (bestMatch) {
                activeVars[bestMatch] = true;
            }
        } else {
            // Single or optional variable - check if any patterns match
            for (const varName of varDef.variables) {
                const patterns = varDef.patterns[varName] || [];
...
/**
 * Check if chapter-local variable patterns exist in rendered text.
 * @param {Array} patterns - Array of text patterns to check
 * @param {string} targetVersionId - Version to check
 * @returns {boolean}
 */
function checkChapterLocalPatternsInVersionText(patterns, targetVersionId) {
    ...
    for (const para of paragraphsToSearch) {
        ...
        for (const pattern of patterns) {
            const cleanPattern = normalizeQuotes(pattern.replace(/\s+/g, ' ').trim());
            if (cleanPattern.length >= 3 && cleanText.includes(cleanPattern.substring(0, 50))) {
                return true;
            }
        }
    }
...
function renderVariablesPanel() {
        ...
        return vars.map(v => {
            ...
            if (inCurrentChapter) {
                if (patternsFound) {
                    ...
                    chapterHint = ' (active but not rendered in this chapter)';
                    notRendered = 'var-tag-not-rendered';
                }
            }
    ...
    container.querySelectorAll('.macro-tag').forEach(tag => {
        tag.addEventListener('click', () => {
            ...
            openChapterMacroInspector(macroName, chapterId);
        });
    });
...
/**
 * Open the Macro Inspector modal for a chapter-local macro.
 */
function openChapterMacroInspector(macroName, chapterId) {
    // Get macro info from chapter_macros
    const macroInfo = chapterMacros[chapterId]?.[macroName];
    if (!macroInfo) {
        showNotification(`Macro "${macroName}" not found in ${chapterId}`, 'error');
        return;
    }

    // Create or get the macro inspector modal
    let modal = document.getElementById('chapter-macro-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'chapter-macro-modal';
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 600px;">
                <div class="modal-header">
                    <h2>Macro: <span id="macro-modal-name"></span></h2>
                    <button class="close-btn" onclick="closeChapterMacroModal()">&times;</button>
                </div>
                <div class="modal-body">
                    <div id="macro-modal-source" class="source-content"></div>
                    <div id="macro-modal-info" style="margin-top: 1rem; font-size: 0.9em; color: var(--text-muted);"></div>
                </div>
            </div>
        `;
        ...
        modal.addEventListener('click', (e) => {
            if (e.target === modal) closeChapterMacroModal();
        });
    }

    // Populate the modal
    const nameEl = document.getElementById('macro-modal-name');
    const sourceEl = document.getElementById('macro-modal-source');
    const infoEl = document.getElementById('macro-modal-info');

    nameEl.textContent = macroName;

    // Format the raw macro definition
    const rawDef = `[MACRO ${macroName}][${macroInfo.raw}]`;
    sourceEl.innerHTML = highlightQuantSyntax(rawDef);

    // Build info section
    let infoHtml = `<p><strong>Chapter:</strong> ${chapterId}</p>`;
    infoHtml += `<p><strong>Variants:</strong> ${macroInfo.variant_count || macroInfo.variants?.length || '?'}</p>`;

    if (macroInfo.references_vars?.length > 0) {
        infoHtml += `<p><strong>References variables:</strong> ${macroInfo.references_vars.join(', ')}</p>`;
    }
    if (macroInfo.references_macros?.length > 0) {
        infoHtml += `<p><strong>References macros:</strong> ${macroInfo.references_macros.join(', ')}</p>`;
    }
    if (macroInfo.has_probabilities) {
        infoHtml += `<p><em>Has probability weights</em></p>`;
    }
...
function closeChapterMacroModal() {
    const modal = document.getElementById('chapter-macro-modal');
    if (modal) modal.classList.add('hidden');
}

function setupVarTagTooltipPositioning() {
    document.querySelectorAll('.var-tag[data-tooltip]').forEach(tag => {
        // Skip if already has listener
        if (tag.dataset.tooltipPositioned) return;
        tag.dataset.tooltipPositioned = 'true';

        tag.addEventListener('mouseenter', () => {
            // Remove any existing positioning classes
            tag.classList.remove('tooltip-align-left', 'tooltip-align-right');

            const rect = tag.getBoundingClientRect();
            const viewportWidth = window.innerWidth;
            const tooltipMaxWidth = 280; // matches CSS max-width
            const margin = 16; // buffer from edge

            // Check if tooltip would overflow on the left when centered
            const tooltipCenterX = rect.left + (rect.width / 2);
            const tooltipHalfWidth = tooltipMaxWidth / 2;

            if (tooltipCenterX - tooltipHalfWidth < margin) {
                // Too close to left edge - align tooltip to left
                tag.classList.add('tooltip-align-left');
            } else if (tooltipCenterX + tooltipHalfWidth > viewportWidth - margin) {
                // Too close to right edge - align tooltip to right
                tag.classList.add('tooltip-align-right');
            }
            // Otherwise, default centered positioning applies
        });
    });
}
...
function handleVariableClick(varName, targetVersion = null, isChapterLocal = false) {
    if (!hasSeenVarHighlightExplanation) {
        // Show first-time explanation modal
        showVarHighlightExplanation(varName, targetVersion, isChapterLocal);
    } else {
        // Proceed directly with highlighting
        highlightVariableText(varName, targetVersion, isChapterLocal);
    }
}

function showVarHighlightExplanation(varName, targetVersion = null, isChapterLocal = false) {
    ...
    // Store the variable name and target version to highlight after closing
    modal.dataset.pendingVar = varName;
    modal.dataset.pendingTargetVersion = targetVersion || '';
    modal.dataset.pendingIsChapterLocal = isChapterLocal ? 'true' : 'false';
...
function closeVarHighlightExplanation(proceed = false) {
    ...
    const pendingTargetVersion = modal.dataset.pendingTargetVersion || null;
    const pendingIsChapterLocal = modal.dataset.pendingIsChapterLocal === 'true';
    ...
    if (proceed && pendingVar) {
        highlightVariableText(pendingVar, pendingTargetVersion, pendingIsChapterLocal);
    }
}
 ...
 * Called when panels are closed, view mode changes, or chapter changes.
 */
...
function highlightVariableText(varName, targetVersion = null, isChapterLocal = false) {
    ...
    // Get patterns - either from chapter-local variables or global variables
    let patterns = [];

    if (isChapterLocal) {
        // Look up patterns from chapter-local variable definitions
        const varDefs = chapterVariables[currentChapter] || [];
        for (const varDef of varDefs) {
            if (varDef.variables.includes(varName) && varDef.patterns?.[varName]) {
                patterns = varDef.patterns[varName];
                break;
            }
        }
    } else {
        // Look up patterns from global variables
        ...
        patterns = info.patterns?.[currentChapter] || [];
    }
    ...
    // If targetVersion is specified, only search in that version's panel (side-by-side view)
    let paragraphs;
    if (targetVersion) {
        const versionPanels = container.querySelectorAll('.version-panel');
        if (versionPanels.length >= 2) {
            // Determine which panel (0=versionA, 1=versionB) matches targetVersion
            let targetPanel = null;
            if (targetVersion === versionA && versionPanels[0]) {
                targetPanel = versionPanels[0];
            } else if (targetVersion === versionB && versionPanels[1]) {
                targetPanel = versionPanels[1];
            }
            if (targetPanel) {
                paragraphs = targetPanel.querySelectorAll('p, .comparison-paragraph, .source-paragraph');
            } else {
                // Fallback: search all paragraphs if panel not found
                paragraphs = container.querySelectorAll('p, .comparison-paragraph, .source-paragraph');
            }
        } else {
            // Not in side-by-side view - search all paragraphs
            paragraphs = container.querySelectorAll('p, .comparison-paragraph, .source-paragraph');
        }
    } else {
        // No target version specified - search all paragraphs
        paragraphs = container.querySelectorAll('p, .comparison-paragraph, .source-paragraph');
    }
    let matchCount = 0;
    let firstMatch = null;
    ...
    if (matchCount > 0) {
        ...
        // More informative message: the variable is active but text wasn't rendered
        showNotification(
            `Variable "${varName}" is active for this seed, but its text may not appear due to other conditional choices in this chapter.`,
            'info'
        );
    }
}
...
function findVariableSourceSnippet(varName) {
    ...
    const snippets = [];
    const addedLineNumbers = new Set(); // Avoid duplicate snippets

    // Pattern to match variable conditionals: [@varname> or [^@varname> or [*tag*@varname> or |@varname>
    // Also matches alternatives in MACRO definitions like |@varname>
    const varPattern = new RegExp(`(?:\\[|\\|)(?:\\*\\w+\\*)?[\\^]?@${varName}>`, 'i');

    for (let i = 0; i < lines.length; i++) {
        if (varPattern.test(lines[i]) && !addedLineNumbers.has(i)) {
    ...
    // Check for DEFINE statements in the chapter (for chapter-local variables)
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.includes('[DEFINE') && line.toLowerCase().includes(`@${varName.toLowerCase()}`)) {
            if (!addedLineNumbers.has(i)) {
                // Get preceding comment for context
                let contextStart = i;
                while (contextStart > 0 && lines[contextStart - 1].startsWith('#')) {
                    contextStart--;
                }
                const snippet = lines.slice(contextStart, i + 1).join('\n');
                snippets.push({
    ...
    // Check for MACRO definitions in the chapter (for chapter-local macros using this variable)
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.includes('[MACRO') && line.toLowerCase().includes(`@${varName.toLowerCase()}`)) {
            if (!addedLineNumbers.has(i)) {
                snippets.push({
                    lineNumber: i + 1,
                    snippet: line,
...
function showVariableSourcePanel(varName, anchorElement) {
    ...
    if (chapterSnippets.length > 0) {
        const sourceKey = SOURCE_KEY_BY_CHAPTER[currentChapter];
        const chapterDefSnippets = chapterSnippets.filter(s => s.isDefinition);
        const chapterMacroSnippets = chapterSnippets.filter(s => s.isMacro);
        const chapterUsageSnippets = chapterSnippets.filter(s => !s.isDefinition && !s.isMacro);

        if (chapterDefSnippets.length > 0) {
            html += `<div class="var-source-section"><span class="var-source-label">Definition (${sourceKey}.txt):</span>`;
            chapterDefSnippets.forEach(s => {
                html += `<pre class="var-source-code">${highlightQuantSyntax(s.snippet)}</pre>`;
            });
            html += `</div>`;
        }

        if (chapterMacroSnippets.length > 0) {
            html += `<div class="var-source-section"><span class="var-source-label">Macro (${sourceKey}.txt):</span>`;
            chapterMacroSnippets.forEach(s => {
                html += `<pre class="var-source-code">${highlightQuantSyntax(s.snippet)}</pre>`;
            });
            html += `</div>`;
        }

        if (chapterUsageSnippets.length > 0) {
            html += `<div class="var-source-section"><span class="var-source-label">Usage (${sourceKey}.txt):</span>`;
            chapterUsageSnippets.forEach(s => {
                html += `<pre class="var-source-code">${highlightQuantSyntax(s.snippet)}</pre>`;
            });
            html += `</div>`;
        }
...
function restoreAllToggledParagraphs() {
    // Restore original content to all paragraphs that are showing source
    originalParagraphContent.forEach((content, key) => {
        const [versionId, chapter, index] = key.split('-');
        const para = findParagraphElement(versionId, parseInt(index, 10));
        if (para) {
            para.innerHTML = content;
            para.classList.remove('paragraph-showing-source');
            // Re-add the source toggle button
            addSourceToggleIcon(para);
        }
    });
}

function findParagraphElement(versionId, paragraphIndex) {
    const container = document.getElementById('comparison-display');
    if (!container) return null;

    // Try to find by data attributes
    return container.querySelector(`[data-version-id="${versionId}"][data-paragraph-index="${paragraphIndex}"]`);
}

function addSourceToggleIcons(container) {
    if (!sourceCodeModeEnabled) return;
    if (!container) return;

    // Pre-compute which paragraphs have source available (one alignment per version, not per paragraph)
    const sourceAvailability = computeSourceAvailability();

    // Select all paragraph elements that have paragraph index data
    const paragraphs = container.querySelectorAll('p[data-paragraph-index], .comparison-paragraph[data-paragraph-index]');
    paragraphs.forEach(para => {
        // Skip source paragraphs (when version is Source Code)
        if (para.classList.contains('source-paragraph')) return;
        // Skip if already showing source
        if (para.classList.contains('paragraph-showing-source')) return;
        // Skip placeholder cells in collation view
        if (para.classList.contains('placeholder')) return;

        // For diff view paragraphs without versionId, use versionA as default
        if (!para.dataset.versionId && currentMode === 'diff') {
            para.dataset.versionId = versionA;
        }

        addSourceToggleIcon(para, sourceAvailability);
    });
}
...
function sourceHasQuantSyntax(rawSourceText, conditionalContext = null) {
    ...
    if (/\[(?:\*\w+\*)?[\^]?@\w+>/.test(rawSourceText)) return true;

    // Check for probability markers: [50> or |25>
    if (/[\[|]\d+>/.test(rawSourceText)) return true;

    // Check for macro references: {MacroName} but not just {i/...} or {b/...}
    // Look for { followed by a word that's not a known formatting command
    if (/\{(?!i\/|b\/|vspace|chapter|part|epigraph|end_)[a-zA-Z][\w\s]*[\/}]/.test(rawSourceText)) return true;

    // Check for alternatives with | inside brackets (but not in DEFINE/MACRO declarations)
    // This catches things like [text1|text2] choices
    if (/\[[^\[\]]*\|[^\[\]]*\]/.test(rawSourceText) && !/\[(DEFINE|MACRO)/.test(rawSourceText)) return true;

    // Check for negated conditionals: [^text
    if (/\[\^[^@]/.test(rawSourceText)) return true;

    return false;
}

// Pre-compute which paragraphs have source available using position-based mapping
function computeSourceAvailability() {
    ...
    if (currentMode === 'diff') {
        ...
        const paragraphsB = chapterDataB ? chapterDataB.paragraphs : [];

        let diffAlignments;
        if (versionC) {
            const chapterDataC = getChapterContent(versionC, currentChapter);
            const paragraphsC = chapterDataC ? chapterDataC.paragraphs : [];
            diffAlignments = alignThreeParagraphs(paragraphsA, paragraphsB, paragraphsC);
        } else {
...
function addSourceToggleIcon(paragraph, sourceAvailability = null) {
    // Skip if already has icon
    if (paragraph.querySelector('.source-toggle-btn')) return;
    // Skip source paragraphs
    if (paragraph.classList.contains('source-paragraph')) return;

    const versionId = paragraph.dataset.versionId;
    const paragraphIndex = parseInt(paragraph.dataset.paragraphIndex, 10);
    const originalIndex = paragraph.dataset.originalIndex || null;

    // Check if source is available using pre-computed map (if provided)
    // If sourceAvailability is null, skip the check (caller knows source is available)
    if (sourceAvailability) {
        let hasSource = false;
        if (currentMode === 'comparison' && originalIndex !== null && originalIndex !== '') {
            hasSource = sourceAvailability.has(`${versionId}-orig-${originalIndex}`);
        } else {
            hasSource = sourceAvailability.has(`${versionId}-${paragraphIndex}`);
        }
        if (!hasSource) return;
    }

    // Make paragraph position relative if needed
    const currentPosition = window.getComputedStyle(paragraph).position;
    if (currentPosition === 'static') {
        paragraph.style.position = 'relative';
    }
    ...
    btn.title = 'Toggle source code view';

    // Check if this paragraph is currently showing source
    const key = `${versionId}-${currentChapter}-${paragraphIndex}`;
    if (toggledSourceParagraphs.has(key)) {
        btn.classList.add('active');
    }

    btn.addEventListener('click', (e) => {
        e.stopPropagation();
        toggleParagraphSource(paragraph);
    });
...
function toggleParagraphSource(paragraph) {
    const versionId = paragraph.dataset.versionId;
    const paragraphIndex = parseInt(paragraph.dataset.paragraphIndex, 10);
    const key = `${versionId}-${currentChapter}-${paragraphIndex}`;

    if (toggledSourceParagraphs.has(key)) {
        // Restore rendered text
        restoreRenderedParagraph(paragraph, key);
        toggledSourceParagraphs.delete(key);
        // Get the NEW button (added by restoreRenderedParagraph) and ensure it's not active
        const btn = paragraph.querySelector('.source-toggle-btn');
        if (btn) btn.classList.remove('active');
    } else {
        // Show source code
        showParagraphSource(paragraph, versionId, paragraphIndex, key);
        toggledSourceParagraphs.add(key);
        const btn = paragraph.querySelector('.source-toggle-btn');
        if (btn) btn.classList.add('active');
    }
}

function showParagraphSource(paragraph, versionId, paragraphIndex, key) {
    // Remove the source toggle button before saving (so it's not included in saved content)
    const existingBtn = paragraph.querySelector('.source-toggle-btn');
    if (existingBtn) existingBtn.remove();

    // Store original content for restoration (without the button)
    originalParagraphContent.set(key, paragraph.innerHTML);

    // Get the original index for collation view (if available)
    const originalIndex = paragraph.dataset.originalIndex || null;

    // Get the corresponding source paragraph
    const sourceHtml = getSourceForRenderedParagraph(versionId, paragraphIndex, originalIndex);

    if (!sourceHtml) {
        showNotification('Source not available for this paragraph', 'info');
        return;
    }

    // Replace paragraph content with source
    paragraph.innerHTML = sourceHtml;
    paragraph.classList.add('paragraph-showing-source');

    // Re-add the toggle button (now active)
    addSourceToggleIcon(paragraph);
    const btn = paragraph.querySelector('.source-toggle-btn');
    if (btn) btn.classList.add('active');
}

function restoreRenderedParagraph(paragraph, key) {
    const originalContent = originalParagraphContent.get(key);
    if (originalContent) {
        paragraph.innerHTML = originalContent;
        paragraph.classList.remove('paragraph-showing-source');
        originalParagraphContent.delete(key);

        // Re-add the source toggle button (since innerHTML wiped it)
        if (sourceCodeModeEnabled) {
            addSourceToggleIcon(paragraph);
        }
    }
...
function getSourceForRenderedParagraph(versionId, paragraphIndex, originalIndex = null) {
    ...
    if (currentMode === 'diff') {
        ...
        if (versionC) {
            const chapterDataC = getChapterContent(versionC, currentChapter);
            const paragraphsC = chapterDataC ? chapterDataC.paragraphs : [];
            alignments = alignThreeParagraphs(paragraphsA, paragraphsB, paragraphsC);
        } else {
            alignments = alignParagraphs(paragraphsA, paragraphsB);
        }
    ...
    if (rawSource) {
        ...
        function lookupMacro(name) {
            ...
            if (globalMacros.has(name)) {
                return globalMacros.get(name);
            }
            return null;
        }

        for (const macroName of macroRefs) {
...
// Get full source data including content-only paragraphs and macro definitions
function getSourceChapterData(chapterId) {
    const key = getSourceKeyForChapter(chapterId);
    if (!key || !originSourcesLoaded || !originSources || !originSources.chapters[key]) {
        return null;
    }
...
// Get global macros from globals.txt (cached)
function getGlobalMacros() {
    if (globalMacrosCache !== null) {
        return globalMacrosCache;
    }

    globalMacrosCache = new Map();

    if (!originSourcesLoaded || !originSources || !originSources.chapters['globals']) {
        return globalMacrosCache;
    }
...
// Find macro references like {MacroName} in source text
function findMacroReferences(text) {
    const macros = [];
    const regex = /\{([^}\/]+)\}/g;
    let match;
    while ((match = regex.exec(text)) !== null) {
        const macroName = match[1].trim();
        // Skip inline formatting like {i/text}
        if (!macroName.includes('/') && macroName !== 'pp' && macroName !== 'chapter') {
...
function displayComparison() {
    ...
    // Clear variable highlights when changing view
    clearVariableHighlights();
...
function displayParagraphComparisonTwoWay(div, paragraphsA, paragraphsB) {
    ...
    alignments.forEach((alignment, rowIndex) => {
        ...
        divA.dataset.versionId = versionA;
        divA.dataset.originalIndex = indexA !== null ? indexA : '';
        if (textA) {
        ...
        divB.dataset.versionId = versionB;
        divB.dataset.originalIndex = indexB !== null ? indexB : '';
        if (textB) {
...
function createCollationCell(text, index, version, type, isPlaceholder, rowIndex) {
    ...
    cell.dataset.originalIndex = index !== null ? index : '';
...
async function loadVariableInfo() {
    try {
        ...
        if (data.variables) {
            ...
            chapterMacros = data.chapter_macros || {};
        } else {
        ...
        const chapterMacroCount = Object.values(chapterMacros).reduce((sum, m) => sum + Object.keys(m).length, 0);
        console.log(`Loaded info for ${Object.keys(variableInfo).length} variables, ${variableGroups.length} groups, ${chapterVarCount} chapter-local vars, ${chapterMacroCount} chapter-local macros`);
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let tapestrySelectedBundles = []; // Array of up to 3 selected bundle indices
    ...
    function handleTapestryKeydown(e) {
        if (e.key === 'Escape') {
            // Progressive de-escalation: welcome → selections → close
            const welcome = document.getElementById('tapestry-welcome');
            if (welcome && !welcome.classList.contains('hidden')) {
                dismissTapestryWelcome();
            } else if (tapestrySelectedBundles.length > 0) {
                tapestrySelectedBundles = [];
                hideTapestryDetailPanels();
                updateTapestryThreadAppearance();
            } else {
    ...
    // CSS rules in compare.css handle opacity based on group-level state classes (.t-hover, .t-lock)
    // and per-element classes (.t-active, .t-locked, .t-hidden).
    function updateTapestryThreadAppearance() {
        ...
        groupEl.classList.toggle('t-lock', tapestrySelectedBundles.length > 0);
        ...
        threads.forEach(el => {
            ...
            // Selected threads (up to 3)
            const isSelected = tapestrySelectedBundles.includes(idx);
            el.classList.toggle('t-locked', isSelected);

            // Stroke-width: selected threads are thicker
            if (isSelected) {
    ...
    function toggleTapestryCluster(clusterIndex) {
        ...
        updateClusterFilterButtons();
        // Remove any selected bundles that belong to a hidden cluster
        let changed = false;
        tapestrySelectedBundles = tapestrySelectedBundles.filter(idx => {
            const b = tapestryData.bundles[idx];
            if (b && !tapestryClusterFilter[b.cluster]) { changed = true; return false; }
            return true;
        });
        if (changed) {
            hideTapestryDetailPanels();
            showTapestryDetailPanels();
        }
        updateTapestryThreadAppearance();
    }
    ...
    // Multi-select detail panels (up to 3 bundles)
    function showTapestryDetailPanels() {
        const container = document.getElementById('tapestry-svg-container');
        if (!container || !tapestryData) return;

        // Clear existing panels
        container.querySelectorAll('.tapestry-detail-panel').forEach(p => p.remove());

        if (tapestrySelectedBundles.length === 0) return;

        // Create wrapper for all panels
        let wrapper = container.querySelector('.tapestry-detail-wrapper');
        if (!wrapper) {
            wrapper = document.createElement('div');
            wrapper.className = 'tapestry-detail-wrapper';
        ...
        tapestrySelectedBundles.forEach((bundleIndex) => {
            ...
            const color = TAPESTRY_COLORS[bundle.cluster % TAPESTRY_COLORS.length];
            ...
            panel.innerHTML = `
                <div class="tapestry-detail-header">
                    <span class="tapestry-detail-title" style="color:${color}">${TAPESTRY_CLUSTER_NAMES[bundle.cluster]} \u2014 ${bundle.count.toLocaleString()} copies</span>
                    <button class="tapestry-detail-close" data-deselect="${bundleIndex}">
                        <i data-lucide="x"></i>
                    </button>
                </div>
                <div class="tapestry-detail-body">
                    <div>${((bundle.count / total) * 100).toFixed(1)}% of all copies</div>
                    <div>Rarity: ${minPct}%\u2013${maxPct}% (avg ${avgPct}%)</div>
                    <div>Most unique: ${maxChapter}</div>
                    ${nearest ? `
                        <div class="tapestry-detail-nearest">Most similar seed: ${nearest.id}</div>
                        <button class="tapestry-view-btn" data-version="${nearest.id}" data-chapter="${tapestryData.meta.chapters[maxChIdx]}">
        ...
        // Clear all button (to the right of panels, only if 2+ selected)
        if (tapestrySelectedBundles.length >= 2) {
            const clearBtn = document.createElement('button');
            clearBtn.className = 'tapestry-clear-all-btn';
            clearBtn.innerHTML = '<i data-lucide="x"></i> Clear all';
        ...
        // Wire up close buttons (deselect individual bundles)
        wrapper.querySelectorAll('.tapestry-detail-close[data-deselect]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const idx = parseInt(btn.dataset.deselect, 10);
                tapestrySelectedBundles = tapestrySelectedBundles.filter(i => i !== idx);
                hideTapestryDetailPanels();
                showTapestryDetailPanels();
                updateTapestryThreadAppearance();
            });
        ...
        // Wire up view-in-browser buttons
        wrapper.querySelectorAll('.tapestry-view-btn[data-version]').forEach(btn => {
            btn.addEventListener('click', () => {
                tapestryOpenInBrowser(btn.dataset.version, btn.dataset.chapter);
            });
        });
    ...
    function hideTapestryDetailPanels() {
        const container = document.getElementById('tapestry-svg-container');
        if (!container) return;
        const wrapper = container.querySelector('.tapestry-detail-wrapper');
    ...
    function handleTapestryResize() {
        ...
        tapestryResizeTimer = setTimeout(() => {
            ...
            if (modal && !modal.classList.contains('hidden') && tapestryData) {
                ...
                updateTapestryThreadAppearance();
                updateClusterFilterButtons();
                if (tapestrySelectedBundles.length > 0) {
                    showTapestryDetailPanels();
                }
            }
        }, 250);
    ...
    function renderTapestryDiagram(data) {
        ...
        threadGroup.selectAll('path')
            ...
            .on('mouseleave', function (_event, d) {
                // O(1) class toggle: CSS restores lock/default opacity
                const group = this.parentNode;
                group.classList.remove('t-hover');
                this.classList.remove('t-active');
                // Restore width based on current state
                const isSelected = tapestrySelectedBundles.includes(this.__bundleIdx);
                const w = isSelected ? Math.max(2, Math.sqrt(d.count) * 1.2) : Math.max(0.5, Math.sqrt(d.count) * 0.6);
            ...
            .on('click', function (event) {
                ...
                const alreadySelected = tapestrySelectedBundles.indexOf(i);
                if (alreadySelected !== -1) {
                    // Deselect this bundle
                    tapestrySelectedBundles.splice(alreadySelected, 1);
                } else if (tapestrySelectedBundles.length < 3) {
                    // Select (up to 3)
                    tapestrySelectedBundles.push(i);
                } else {
                    // At max: replace oldest selection
                    tapestrySelectedBundles.shift();
                    tapestrySelectedBundles.push(i);
                }
                hideTapestryDetailPanels();
                showTapestryDetailPanels();
        ...
        if (!existingLegend) {
            ...
            legend.innerHTML = `
                ...
                <div class="tapestry-legend-section" style="color:#888;font-size:0.75rem">
                    <strong>Hover</strong> any thread for details. <strong>Click</strong> up to 3 threads to compare.
